import { UnifiedError } from "../../core/errors.js";
/**
 ...
 */
export type StorageErrorCode = "storage_unavailable" | "storage_read_only" | "storage_not_granted" | "not_found" | "invalid_input" | "aborted";
/** Construct a typed storage error (a plain `UnifiedError`, not an HTTP error). */
export declare const storageError: (code: StorageErrorCode, message: string) => UnifiedError;
/** Construct the `UnifiedError` raised when a caller's `signal` aborts a storage call. */
export declare function storageAbortError(what: string, reason?: unknown): UnifiedError;
/**
 ...
 */
export declare function throwIfAborted(signal: AbortSignal | undefined, what: string, cause?: unknown): void;
