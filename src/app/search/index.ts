// Shared kernel for cross-app search providers — import from
// "@unifiedai/sdk/app".
//
// Every app that ships a `search.js` entry (sheets, docs, design, notes,
// planner) plus the host's built-in calendar provider used to carry its own
// copy of the same five things: the word-boundary term matcher, the weighted
// field scorer, the `hints.ids` check, the `searchText` server-side pushdown
// query, and the one-way capability latch that turns that pushdown off after
// its first failure. The copies were byte-identical where they had not yet
// drifted, and had drifted where it mattered — three different values for the
// single `HINTS_FLOOR` invariant. They live here now; each app keeps only its
// own data fetch and its own `toHit` mapping.
//
// Lives in the SDK (formerly the UnifiedApp workspace's @unified/app-search
// source alias) so the UnifiedApp host, its bundled apps, AND third-party
// marketplace apps all share one copy.
//
// Deliberately DEPENDENCY-FREE. The app search entries never import the SDK's
// runtime (they reach the host through the externalized `@unified/host-api`),
// so a helper here must not drag SDK code into the `search.js` chunk that
// exists precisely to stay small. Nothing in this file imports any other SDK
// module — not even as a type; the one place an SDK type would be natural
// (`Collection<T>.query`) is matched structurally instead, see
// `SearchTextCollection`. The provider contract types themselves
// (src/resources/search/types.ts) are re-exported from the "./app" barrel,
// which is types-only traffic and adds nothing to a bundle.
 ...
 */
export function matchKind(haystackLower: string, term: string): MatchKind {
  if (!term) return "none";
  let idx = haystackLower.indexOf(term);
  if (idx === -1) return "none";
  let sawMid = false;
  while (idx !== -1) {
    const isBoundary = idx === 0 || BOUNDARY_BEFORE.test(haystackLower[idx - 1] as string);
 ...
 */
export function scoreFields(fields: ScoreFields, terms: string[], query: string): number {
  ...
  for (const term of terms) {
    ...
    if (titleMatch === "boundary") score += 3;
    else if (titleMatch === "mid") score += 1.5;
    else allInTitle = false;

    const secondaryMatch = matchKind(secondaryLower, term);
    if (secondaryMatch === "boundary") score += 1;
    else if (secondaryMatch === "mid") score += 0.5;

    const bodyMatch = matchKind(bodyLower, term);
    if (bodyMatch === "boundary") score += 0.75;
    else if (bodyMatch === "mid") score += 0.25;
  }
  if (allInTitle) score += 2;
  if (titleLower === query.toLowerCase()) score += 5;
  return score;
}
 ...
 */
export function isHinted(id: string, hintIds: string[] | undefined, appId: string): boolean {
  if (!hintIds || hintIds.length === 0) return false;
  return hintIds.includes(id) || hintIds.includes(`${appId}:${id}`);
}

// ─── Server-side `searchText` pushdown ───────────────────────────────────────
 ...
 */
export async function queryBySearchText<Row>(
  collection: SearchTextCollection<Row> | null | undefined,
  text: string,
  limit: number,
): Promise<Row[]> {
 ...
 */
export function createLatchedFallback<Row>(
  label: string,
  fn: (query: string, limit: number) => Promise<Row[]>,
): LatchedFallback<Row> {
  let disabled = false;
  return {
    ...
    async run(query: string, limit: number, signal?: AbortSignal): Promise<Row[]> {
      if (disabled) return [];
      ...
      try {
        ...
        disabled = true;
        console.warn(
          `[${label}] server-side searchText match unavailable — falling back to a widened client-side scan for this session`,
          err,
        );
        return [];
      }
 ...
 * already did.
 */
export function toSearchHit(input: SearchHitInput): AppSearchHit {
 ...
 */
export function compareHits<C extends { score: number }>(
  updatedAtOf: (c: C) => number,
): (a: C, b: C) => number {
