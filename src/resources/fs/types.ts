// Public types + the swappable backend contract for `sdk.fs` — the local-first,
// app-namespaced FILE workspace. The sibling of `sdk.storage`: where storage is
// a typed key/value+blob store, `fs` is a jailed directory tree an app (and the
// agent loop running on its behalf) reads, writes, and edits.
//
...
// re-validates — confining every op WITHIN `app-fs/<ns>/` (lexical + symlink
// jail) regardless of what JS sends. The SDK-side checks are defense-in-depth.
// NOTE: the `ns` itself is still cooperative today — the host does NOT yet
// re-derive it from an attributed app identity, and there is no enforced
// cross-app gate; that hard authorization is a future broker concern. See
